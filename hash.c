...
/*End of prefix code. Do not edit code above this line*/
	...
	//Complete rest of the code for search()
        for(i=0; i<M; i++){
        	if(hashTable[(pos+i)%M] == FREE)
			{;}
		else{ 
			if(hashTable[(pos+i)%M] == EMPTY){				
				return -1;
			}
			
        	 	else{
				if(hashTable[(pos+i)%M] == hash){				
					return (pos+i)%M;
				}
		 	    }
		    }
...
int insert(char *a)
{
	...
	int i, s=0;	
	for(i=0; i<M; i++){
        	if(hashTable[(pos+i)%M] == EMPTY || hashTable[(pos+i)%M] == FREE){ 
        	 	s = (pos+i)%M;
			hashTable[s] = hash;
			strcpy(valTable[s].value,a);
			return s;
		}
